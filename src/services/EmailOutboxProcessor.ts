...
export class EmailOutboxProcessor {
  ...
  private async processEvent(
    ...
    try {
      ...
      const updated = await this.outboxStore.markPublished(event.id, this.workerId);
      ...
      await this.outboxStore.markDeliveryFailed({
        ...
        advanceIdempotencyGeneration: decision.advanceIdempotencyGeneration,
      });

      return decision.permanentlyFailed ? "permanentlyFailed" : "retried";
    }
  }
  ...
  async processBatch(): Promise<ProcessBatchResult> {
    ...
    return {
      ...
      permanentlyFailed: outcomes.filter((outcome) => outcome === "permanentlyFailed")
        .length,
    };
  }
}
