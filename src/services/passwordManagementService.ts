...
import type { PasswordResetRecord } from "../repositories/passwordResetRepository.js";
...
export async function requestPasswordReset(email: string): Promise<void> {
  ...
  try {
    ...
    await passwordResetRepository.revokeUnusedTokens(client, currentUser.id);
...
export async function validatePasswordResetToken(
  ...
  const reset = await passwordResetRepository.findByTokenHash(hashToken(token));
...
export async function resetPassword(
  ...
  try {
    ...
    if (settings.terminateSessionsOnPasswordReset) {
      await sessionRepository.revokeUserSessions(client, currentReset.userId);
    }
...
export async function changePassword({
  ...
  try {
    ...
    if (settings.terminateOtherSessionsOnPasswordChange) {
      await sessionRepository.revokeUserSessions(client, user.id, sessionId);
    }

    await outboxRepository.enqueueEvent(client, "password.changed", user.id, {
      userId: user.id,
      email: user.email,
      channel: "email",
    });
