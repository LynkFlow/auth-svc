...
const envSchema = z
  .object({
    ...
    JWT_PRIVATE_KEY_PATH: z.string().min(1).default(".secrets/jwt-private.pem"),
    JWT_PUBLIC_KEY_PATH: z.string().min(1).default(".secrets/jwt-public.pem"),
    ...
    JWT_ACCESS_TOKEN_MINUTES: z.coerce.number().int().min(1).max(60).default(15),
    ...
    EMAIL_OUTBOX_POLL_INTERVAL_MS: z.coerce
      ...
      .max(60_000)
      .default(2_000),
    EMAIL_OUTBOX_BATCH_SIZE: z.coerce.number().int().min(1).max(100).default(10),
    EMAIL_OUTBOX_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(100).default(10),
...
const config = Object.freeze({
  ...
  emailOutboxLockTimeoutSeconds: parsed.data.EMAIL_OUTBOX_LOCK_TIMEOUT_SECONDS,
