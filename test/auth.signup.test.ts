...
test("creates a pending account and queues a usable activation token", async () => {
    ...
    assert.equal(
        ...
        createHash("sha256")
            .update(verification.eventPayload.token)
            .digest("hex"),
    );
    assert.equal(
...
test("maps every supported account type to its business role", async () => {
    const mappings = [
...
test("rejects duplicate email addresses without creating partial records", async () => {
...
test("rejects weak passwords before creating an account", async () => {
