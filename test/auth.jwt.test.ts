...
function extractRefreshCookie(response: request.Response): {
  ...
  assert.ok(cookies);
  const refreshCookie = cookies.find((cookie) => cookie.startsWith("lf.refresh="));
...
describe("JWT issuance, refresh rotation, and reuse detection", () => {
  beforeAll(async () => {
  ...
  it("publishes the RSA public key without exposing private key material", async () => {
  ...
  it("issues a 15-minute access JWT with the expected identity claims", async () => {
    const user = await createUser("claims");
    const authentication = await login(user);
    const principal = await tokenService.verifyAccessToken(authentication.accessToken);
  ...
  it("rotates the refresh token and records the used credential", async () => {
  ...
  it("rejects a forged refresh token without revoking the valid session", async () => {
    ...
    assert.equal(forgedResponse.body.error.code, "AUTH_REFRESH_TOKEN_INVALID");
  ...
  it("detects refresh-token reuse and revokes the token family", async () => {
    ...
    assert.equal(replayResponse.body.error.code, "AUTH_REFRESH_TOKEN_REUSED");

    const rotatedResponse = await request(app)
      .post("/api/v1/auth/token/refresh")
      .set("Cookie", rotated.cookie);
    assert.equal(rotatedResponse.status, 401);
    assert.equal(rotatedResponse.body.error.code, "AUTH_REFRESH_TOKEN_INVALID");
  ...
  it("a forged logout cookie cannot revoke a legitimate session", async () => {
  ...
  it("logout with a previously rotated token revokes its token family", async () => {
    ...
    assert.equal(afterLogout.body.error.code, "AUTH_REFRESH_TOKEN_INVALID");
  });
  ...
  it("rejects expired and wrong-audience JWTs", async () => {
    ...
    assert.equal(expiredResponse.body.error.code, "AUTH_ACCESS_TOKEN_EXPIRED");
    assert.equal(wrongAudienceResponse.status, 401);
    assert.equal(wrongAudienceResponse.body.error.code, "AUTH_ACCESS_TOKEN_INVALID");
  });
});
