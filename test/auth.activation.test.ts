...
describe("account activation", () => {
  ...
  it("returns account information for a valid activation token", async () => {
    ...
    assert.equal(response.body.data.account.organizationName, "Test Organization");
  ...
  it("rejects an invalid activation token", async () => {
    const response = await request(app)
      .post("/api/v1/auth/activation/validate")
  ...
  it("rejects an expired activation token", async () => {
    ...
    const response = await request(app).post("/api/v1/auth/activation/complete").send({
      token,
    ...
    assert.equal(response.body.error.message, "The activation link has expired.");
  });

  it("requires matching passwords and both agreements", async () => {
    ...
    // Field-level validation errors are now a field-keyed record
    // (fieldErrors), not an array of {field, message} objects -- see
    // validate.ts/errorHandler.ts's @lynkflow/types ApiError wiring.
    assert.deepEqual(
      Object.keys(response.body.error.fieldErrors as Record<string, string>),
  ...
  it("requires a password for invited accounts that do not have one", async () => {
    ...
    assert.equal(response.status, 400);
    assert.equal(response.body.error.code, "AUTH_ACTIVATION_PASSWORD_REQUIRED");
  });

  it("activates the account, records consent, and queues notification", async () => {
  ...
  it("allows only one concurrent activation attempt to succeed", async () => {
