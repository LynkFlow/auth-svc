...
describe("password reset and change", () => {
  ...
  it("forgot password uses the same response for known and unknown emails", async () => {
    ...
    const { rows } = await pool.query<
      { tokenCount: number; eventCount: number } & QueryResultRow
    >(
      `
            SELECT
  ...
  it("forgot password does not create tokens for non-active accounts", async () => {
  ...
  it("a newer forgot-password request invalidates the previous link", async () => {
    ...
    assert.equal(firstResponse.body.error.code, "AUTH_PASSWORD_RESET_TOKEN_INVALID");
  ...
  it("rejects expired password-reset links", async () => {
    ...
    assert.equal(response.body.error.code, "AUTH_PASSWORD_RESET_TOKEN_EXPIRED");
    assert.equal(response.body.error.message, "Password reset link has expired.");
  });

  it("reset password requires matching, policy-compliant passwords", async () => {
    ...
    const policyResponse = await request(app).post("/api/v1/auth/password/reset").send({
      token,
      newPassword: "weakpassword",
      confirmPassword: "weakpassword",
    });
    assert.equal(policyResponse.status, 400);
    assert.equal(policyResponse.body.error.code, "AUTH_PASSWORD_POLICY_VIOLATION");
  });

  it("reset password rejects the user's current password", async () => {
  ...
  it("reset password updates credentials, consumes links, and revokes sessions", async () => {
    ...
    assert.equal(
      await passwordService.verifyPassword(verification.passwordHash, newPassword),
      true,
    );
  ...
  it("change password requires an authenticated session", async () => {
  ...
  it("change password rejects an incorrect current password", async () => {
    ...
    assert.equal(response.status, 400);
    assert.equal(response.body.error.code, "AUTH_CURRENT_PASSWORD_INCORRECT");
  });

  it("change password rejects the existing password as the new password", async () => {
  ...
  it("change password preserves the current session and revokes other sessions and reset links", async () => {
    ...
    assert.equal(response.status, 200);
    assert.equal(response.body.message, "Your password has been changed successfully.");
