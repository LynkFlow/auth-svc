...
import type {
    DeliveryFailure,
    OutboxEvent,
} from "../src/repositories/outboxRepository";
import type { EmailSender } from "../src/services/emailServiceClient";
import {
    EmailServiceClient,
    EmailServiceResponseError,
    EmailServiceTransportError,
} from "../src/services/emailServiceClient";
import { mapOutboxEventToEmail } from "../src/services/emailEventMapper";
import {
    EmailOutboxProcessor,
    type OutboxStore,
} from "../src/services/emailOutboxProcessor";
...
test("maps every auth email event to the notification-service contract", () => {
...
test("publishes a delivered event using a stable idempotency key", async () => {
    const item = event();
...
test("keeps the idempotency key after an ambiguous transport failure", async () => {
...
test("advances the idempotency generation after a conflict response", async () => {
    const store = new FakeOutboxStore([event({ deliveryAttempts: 2 })]);
    const emailClient: EmailSender = {
        async send() {
            throw new EmailServiceResponseError(
                409,
...
test("dead-letters malformed events without calling the email service", async () => {
    const store = new FakeOutboxStore([
        event({ payload: { email: "not-an-email" } }),
    ]);
    let sendCalls = 0;
    const emailClient: EmailSender = {
        async send() {
...
test("dead-letters expired token emails instead of sending stale links", async () => {
    const store = new FakeOutboxStore([
        event({
            payload: {
                email: "user@example.com",
...
test("email client posts JSON and the outbox idempotency key", async () => {
    let receivedUrl = "";
    ...
    assert.equal(
        receivedUrl,
        "http://localhost:3010/api/v1/emails/send",
    );
    ...
    assert.deepEqual(JSON.parse(String(receivedOptions?.body)), request);
});
